...
def buymethod2(i):
    stut1 = ls_oclo[i+2] > ls_oclo[i+1+2] and ls_oclo[i+2+2] < ls_oclo[i+3+2] and Shortpossition == False and (ls_oclo[i+3] < ls_bollow[i+3]  or ls_oclo[i+4] < ls_bollow[i+4])
    stut2 = False
    k = 1
    tendp = 0
    Alltendency = []
...
def sellmethod2(i):
    stut1 = ls_ochi[i+2] < ls_ochi[i+1+2] and ls_ochi[i+2+2] > ls_ochi[i+3+2] and Longpossition == False and (ls_ochi[i+3] > ls_bolhigh[i+3] or ls_ochi[i+4] > ls_bolhigh[i+4])
    stut2 = False
    k = 1
    tendp = 0
    Alltendency = []
...
while i < (len(ls_oclo) - 5):
    ...
    if sellmethod2(i):
        ...
        if Shortpossition == True:
            ...
            Shortpossition = False
            proF = round(proF,4)
    if Longpossition == True and ls_low[i+5] < buynum*(1-(1/30)):
        proF = 0
    if Shortpossition == True and ls_high[i+5] > sellnum*(1+(1/30)):
        proF = 0
    answer = answer + ' |' +str(proF)
    print(answer,'|',timefinder_5min(timechecker_5min()-(len(ls_close)-(i+6))), ls_tend1[i] - ls_tend2[i])
